...
def write_txt():
    ...
    for idx, sub in enumerate(subs):
        ...
        clip.export(args.output_dir + args.output_dir + args.audio_out_file_pattern.format(idx), format='wav')
...
def write_csv():
    ...
    with open(os.path.join(args.output_dir, args.csv_filename), 'w', encoding=args.out_encoding) as f:
        for idx, sub in enumerate(subs):
            ...
            start, end = get_slice_indexes(sub)
            clip = audio[start:end]
            clip.export(os.path.join(args.output_dir, args.output_dir + "_" + args.audio_out_file_pattern.format(idx)), format='wav')
            f.write(args.csv_seperator.join([os.getcwd() + "/" + args.output_dir + "/" + args.output_dir + "_" + args.audio_out_file_pattern.format(idx), sub.content.replace('\n', ' ')]))
            f.write('\n')
